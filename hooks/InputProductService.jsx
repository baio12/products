...
function InputProductService({ selectedPerson, setSelectedPerson, people, errors, setErrors }) {
    
    const [filtered, setFiltered] = useState([]);
    const [disabled, setDisabled] = useState(selectedPerson?.product !== '');
    const handleSearch = value => {
        const filteredInfo = people.filter(item => {
            if (value == '') {
                ...
                return item
            } else if (item.prod_id.toLowerCase().includes(value.toLowerCase())) {
                return item
            } else {
                return ''
            }
    ...
    useEffect(() => {
        setDisabled(selectedPerson?.product !== '');
    }, [selectedPerson])

    const handleChange = (e) => {
        setSelectedPerson({ ...selectedPerson, product: e });
        setErrors({ ...errors, product: '' });
        setDisabled(true);
    }

    const handleActivate = () => {
        setSelectedPerson({ ...selectedPerson, product: '' });
        setDisabled(false);
    }
    return (
        <>
            <Combobox as="div" value={selectedPerson?.product} onChange={e => handleChange(e)} className='w-full'>
                <Combobox.Label className="w-full block text-sm font-medium text-gray-700">Clave de producto/servicio <span className='text-red-400'>*</span></Combobox.Label>
                <div className="relative mt-1 outline-none">
                    <Combobox.Input
                        ...
                        displayValue={(item) => item !== '' ? `${item?.prod_id || item?.key} ${item?.name}` : item}
                    ...
                    <Combobox.Options className="absolute z-10 max-h-32 mt-1.5 w-full overflow-y-auto rounded-md bg-white text-sm shadow-lg sm:text-sm ring-1 ring-black ring-opacity-5 focus:outline-none">
                        {filtered.map((item, i) => (
                            <Combobox.Option
                                ...
                                className={({ active }) =>
                                    classNames(
                                        ' scrollbar relative cursor-default select-none overflow-auto py-2.5 px-3 font-medium flex items-center rounded-md',
                                ...
                                {({ active, selected }) => (
                                    <>
                                        <div className="flex items-center">
                                            <span className={classNames('truncate', selected && 'font-semibold')}>{item.prod_id}</span>
                                            <span
                ...
                {errors.product !== '' && <span className='block mt-0.5 text-red-500 text-sm'>{errors.product}</span>}
            </Combobox>
        </>
    )
}

InputProductService.propTypes = {
